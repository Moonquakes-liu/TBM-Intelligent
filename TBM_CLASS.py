...
# * Last update: 2022-10-20 20:00:00                                         *
# * License:  LGPL v1.0                                                      *
...
class TBM_CLASS(object):
    ...
    def create_class_Dir(self):
        ...
        if self.create_dir:
            if not os.path.exists(self.out_path):
                ...
                os.mkdir(self.out_path)
            for number, Dir in zip([i for i in range(len(self.DIR_OUT))], self.DIR_OUT):
                new_dir = '\\%d-' % (number + 1) + Dir
                self.DIR_OUT[number] = new_dir
    ...
    def write_index(self, _inf_, _Num_):
        Root_Path = os.path.dirname(os.path.abspath(__file__))  # 索引文件存放根目录
        except_name = ['A', 'B1', 'B2', 'C1', 'C2', 'D', 'E', '正常掘进段']  # 新添加索引数据标题
    ...
    def data_class(self, _input_path_, _out_path_, _par_name_, _dir_path_):
        """数据分类"""
        if (not _input_path_) and (not _out_path_) and (not _par_name_) and (not _dir_path_):  # 检查传入参数是否正常
        ...
        for num, name in zip([i + 1 for i in range(len(csv_list))], csv_list):
            ...
            if self.A_premature(cycle)[0] == 'Normal':
                ...
                except_type[0] = 'True'
                if_Normal += 1
                shutil.copyfile(local_csv_path, os.path.join(self.out_path + self.DIR_OUT[0], name))
                self.write_index(except_type, int(name[:5]))
                continue
            if self.B1_markAndModify(this_cycle_steady)[0] == 'B1':
                except_type[1] = 'True'
                if_Normal += 1
                shutil.copyfile(local_csv_path, os.path.join(self.out_path + self.DIR_OUT[1], name))
            if self.B2_constant(this_cycle)[0] == 'B2':
                except_type[2] = 'True'
                if_Normal += 1
                shutil.copyfile(local_csv_path, os.path.join(self.out_path + self.DIR_OUT[2], name))
            if self.C1_sine(this_cycle)[0] == 'C1':
                except_type[3] = 'True'
                if_Normal += 1
                shutil.copyfile(local_csv_path, os.path.join(self.out_path + self.DIR_OUT[3], name))
            if self.C2_shutdown(this_cycle_steady)[0] == 'C2':
                except_type[4] = 'True'
                if_Normal += 1
                shutil.copyfile(local_csv_path, os.path.join(self.out_path + self.DIR_OUT[4], name))
            if self.D_adjust_setting(this_cycle_steady)[0] == 'D':
                except_type[5] = 'True'
                if_Normal += 1
                shutil.copyfile(local_csv_path, os.path.join(self.out_path + self.DIR_OUT[5], name))
            if self.E_missing_ratio(this_cycle)[0] == 'E':
                except_type[6] = 'True'
                if_Normal += 1
                shutil.copyfile(local_csv_path, os.path.join(self.out_path + self.DIR_OUT[6], name))
            if not if_Normal:
                except_type[7] = 'True'
                shutil.copyfile(local_csv_path, os.path.join(self.out_path + self.DIR_OUT[7], name))
            self.write_index(except_type, int(name[:5]))
            end = time.time()  # 获取当前时刻时间（用于计算程序执行时间）
            visual(num, len(csv_list), start, end, 'Data-Class')
        visual(-1, len(csv_list), None, None, 'Data-Class', True)
    ...
    def data_clean(self, _input_path_, _out_path_, _par_name_, _dir_path_):
        ...
        for Type, Dir in zip(except_name, os.listdir(_input_path_)):
            ...
            for csv_file in csv_list:
                ...
                if Type == 'Normal':
                    shutil.copyfile(local_csv_path, os.path.join(self.out_path + self.DIR_OUT[-1], csv_file))
                    continue
                else:
                    cycle = pd.read_csv(local_csv_path, encoding='gb2312')
                local = -1
...
def ML_data(_input_path_,  _out_path_):
    """合并形成机器学习数据集"""
    if (not _input_path_) and (not _out_path_):  # 检查传入参数是否正常
        print('->-> %s' % os.path.basename(__file__), '\033[0;31mParameters abnormal, Please check!!!\033[0m')
        sys.exit()
...
def visual(cycle, Sum, start, end, Print, Clear=False):
    """可视化输出"""
    global time_val
    if cycle != -1:
        time_diff = end - start  # 执行一个文件所需的时间
        ...
        print('\r', '->->', '[第%d个 / 共%d个]  ' % (cycle, Sum), '[所用时间%ds / 平均时间%ds]'
              % (int(time_diff), int(mean_time)), '  ', '\033[0;33m累积时间:%6.3f小时\033[0m' % sum_time, end='')
    else:
        sum_time = round(sum(time_val) / 3600, 3)  # 计算程序执行的总时间
