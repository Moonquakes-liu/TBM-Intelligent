...
class TBM_REPORT(object):
    ...
    def cre_pdf(self, _input_file_path_, _input_pic_path_, _out_path_, _par_name_):
        """读取数据，并将数据转化为可识别的类型"""
        if (not _input_file_path_) and (not _input_pic_path_) and (not _out_path_) and (not _par_name_):  # 检查传入参数是否正常
            print('->-> %s' % os.path.basename(__file__), '\033[0;31mParameters abnormal, Please check!!!\033[0m')
            sys.exit()
        self.parameters, self.out_path = _par_name_, _out_path_
        ...
        visual(-1, self.page_sum, None, None, 'PDF generation', True)
...
def plot_parameters_TBM(_input_path_, _out_path_, Par_name):
    ...
    if (not _input_path_) and (not _out_path_) and (not Par_name):  # 检查传入参数是否正常
        print('->-> %s' % os.path.basename(__file__), '\033[0;31mParameters abnormal, Please check!!!\033[0m')
        sys.exit()
    ...
    visual(-1, len(file_name_list), None, None, 'Drawing pic', True)
