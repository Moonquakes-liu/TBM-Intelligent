...
class TBM_REPORT(object):
    ...
    def cre_pdf(self, _input_path_, _input_pic_, _out_path_, _par_name_, History=False):
        """读取数据，并将数据转化为可识别的类型
        :param _input_path_: 文件读取路径
        :param _input_pic_: 图片读取路径
        :param _out_path_: 文件保存路径
        :param _par_name_: 参数名称
        :param History: 展示历史修改记录
        """
        if History: print('\n', '='*100, '\n', HISTORY, '\n', '='*100, '\n')  # 打印文件修改记录
        if (not _input_path_) or (not _input_pic_) or (not _out_path_) or (not _par_name_):  # 检查传入参数是否正常
        ...
        file_name_list = os.listdir(_input_path_)  # 获取循环段列表
        ...
        for cycle, file_name in zip([i + 1 for i in range(len(file_name_list))], file_name_list):
            Data_cycle = pd.read_csv(os.path.join(_input_path_, file_name), encoding='gb2312')  # 读取文件
...
def Plot_Parameters_TBM(_input_path_, _out_path_, _par_name_, History=False):
    """
    ...
    for num, file_name in zip([i + 1 for i in range(len(file_name_list))], file_name_list):  # 遍历每个文件
        ...
        y_n = TBM_Data_cycle.loc[:, _par_name_[0]]  # '刀盘转速'
        y_n_set = TBM_Data_cycle.loc[:, _par_name_[1]]  # '刀盘转速设定'
        y_V = TBM_Data_cycle.loc[:, _par_name_[2]]  # '推进速度'
        y_V_set = TBM_Data_cycle.loc[:, _par_name_[3]]  # '推进速度设定'
        y_T = TBM_Data_cycle.loc[:, _par_name_[4]]  # '刀盘总扭矩'
        y_F = TBM_Data_cycle.loc[:, _par_name_[5]]  # '推进总推力'
