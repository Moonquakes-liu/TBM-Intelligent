...
class TBM_CYCLE(object):
    ...
    必选参数：input_path：原始数据存放路径 [单位：]&[选项：]&[默认：None]。
            out_path：生成数据保存路径 [单位：]&[选项：]&[默认：None]。
            par_name：关键参数名称 [单位：]&[选项：]&[默认：None]。
    可选参数：index_path：索引文件保存路径 [单位：]&[选项：]&[默认：None]。
            division：循环段划分方法 [单位：]&[选项：刀盘转速'Rotate Speed'/推进速度'Advance Speed']&[默认：刀盘转速'Rotate Speed']。
            interval_time：相邻循环段时间间隔 [单位：s]&[选项：]&[默认：100s]。
            V_min：推进速度下限值 [单位：mm/min]&[选项：]&[默认：1]。
            L_min：掘进长度下限值 [单位：mm]&[选项：]&[默认：10]。
            debug：程序调试/修复选项 [单位：]&[选项：True/False]&[默认：False]。
            Run：直接运行程序 [单位：]&[选项：True/False]&[默认：True]。
    """
    ...
    def check_index(self, all_location: list) -> None:
        ...
        if reduce(lambda x, y: x * y, [type(name) == str for name in self.parm]):  # 判断是否为标签类型索引(名称)
            ...
            if mark == 1:  # 标签名称不重复
                ...
                print('-> %s \033[0;31mLabel index is not available, Please enter location index!!!\033[0m' % Class)
    ...
    def write_index(self, info: dict) -> None:  # 规定_Num_为整型(int)，info为列表(list)，返回值返回值无类型限定
        ...
        if self.debug:  # 若为调试模式，则不向索引文件写入内容
            return None
        index_file = 'Resource\\Index-File.csv'
        if self.NEW_INDEX.empty:  # 若新的索引数据为空
            if os.path.isfile(index_file):  # 判断索引文件是否存在
                os.remove(index_file)  # 若索引文件存在，则删除
        Num = int(info['name'][:5])  # 获取当前循环段编号
        self.NEW_INDEX.loc[Num] = [Num] + [info[name] for name in self.INDEX_NAME[1:]]  # 新的索引记录
        # noinspection PyTypeChecker
        self.NEW_INDEX.to_csv(index_file, index=False, encoding='gb2312')  # 保存新的索引记录
    ...
    def default_read(self, file_path: str) -> DataFrame:  # 规定file_path为字符型数据，返回值为（DataFrame）类型数组
        """
        ...
        if os.path.splitext(file_path)[-1] == '.csv':  # 仅读取csv类型文件
            ...
            Class = self.__class__.__name__  # 获取当前模块名称
            print('-> %s \033[0;31mNo csv file is found under the input path!!!\033[0m' % Class)  # csv类型文件不存在，无法继续
            sys.exit()
    ...
    def main(self) -> None:  # 规定返回值无类型限定/无返回值
        ...
        if os.path.isfile('Resource\\Index-File.csv') and not self.debug and self.index:
            shutil.copyfile('Resource\\Index-File.csv', os.path.join(self.index, 'Index-File.csv'))  # 复制索引文件到指定位置
...
class TBM_EXTRACT(object):
    ...
    ****可选参数：（index_path）索引文件保存路径 [单位：]&[选项：]&[默认：None]。
                ...
                Run：直接运行程序 [单位：]&[选项：True/False]&[默认：True]。
    """
    PARAMETERS = ['待提取的关键参数, 可根据需要自行选择']  # 破岩关键数据提取中的参数名称和顺序示例
...
class TBM_CLASS(object):
    ...
    ****必选参数：input_path：原始数据存放路径 [单位：]&[选项：]&[默认：None]。
                ...
                par_name：关键参数名称 [单位：]&[选项：]&[默认：None]。
    ****可选参数：index_path：索引文件保存路径 [单位：]&[选项：]&[默认：None]。
                project_type：工程类型 [单位：]&[选项：'引松'/'引额-361'/'引额-362'/'引绰-667'/'引绰-668']&[默认：'引绰-668']。
                L_min：最小掘进长度 [单位：mm]&[选项：]&[默认：300mm]。
                V_max：推进速度最大值 [单位：mm/min]&[选项：引松120/额河200]&[默认：120]。
                V_set_var：推进速度设定值变化幅值 [单位：]&[选项：True/False]&[默认：15]。
                missing_ratio：数据缺失率 [单位：%]&[选项：True/False]&[默认：20]。
                debug：程序调试/修复选项 [单位：]&[选项：True/False]&[默认：False]。
                Run：直接运行程序 [单位：]&[选项：True/False]&[默认：True]。
    """
    ...
    def __init__(self, input_path=None, out_path=None, index_path=None, parameter=None, project_type='引绰-668',
                 L_min=0.3, V_max=120, V_set_var=15, missing_ratio=20, debug=False, Run=True):
        """初始化必要参量"""
        self.input = input_path  # 初始化输入路径
        self.out = out_path  # 初始化输出路径
        ...
        self.missing_ratio = missing_ratio / 100  # 数据缺失率（单位：%）
        self.debug = debug  # 调试/修复程序
        """初始化程序内部参量"""
        self.Time_val = []  # 初始化程序运行花费时间
        self.show_parm = True  # 将输入参数大音出来，便于进行核对
    ...
    def write_index(self, info: dict) -> None:  # 规定_Num_为整型(int)，info为列表(list)，返回值返回值无类型限定
        ...
        if self.RAW_INDEX.empty:  # 索引文件行位置记录
            if os.path.isfile(index_file):
                self.RAW_INDEX = pd.read_csv(index_file, index_col=0, encoding='gb2312')  # 读取索引文件
        ...
        after_index.to_csv(index_file, index=False, encoding='gb2312')  # 保存新的索引记录
    ...
    def main(self) -> None:  # 规定返回值无类型限定/无返回值
        ...
        for num, file in enumerate(all_file):
            ...
            RS_Index = self.custom_get_RS({'name': file, 'data': data_A})  # 调用自定义函数，获取空推、上升、稳定、下降的变化点
            if RS_Index is None:
                RS_Index = self.default_get_RS({'name': file, 'data': data_A})  # 调用默认函数，获取空推、上升、稳定、下降的变化点
    ...
    def default_get_RS(self, data: dict) -> dict:
        """
    ...
    def custom_get_RS(self, data: dict) -> dict:
    ...
    def write_index(self, info: dict) -> None:  # 规定_Num_为整型(int)，info为列表(list)，返回值返回值无类型限定
        ...
        try:
            if self.RAW_INDEX.empty:  # 索引文件行位置记录
                ...
                self.RAW_INDEX = pd.read_csv(index_file, index_col=0, encoding='gb2312', dtype=Type)  # 读取索引文件
            ...
            if (self.RAW_INDEX.loc[int(info['name'][:5]), self.INDEX_NAME[1:-1]] == 'True').sum() <= 0:
                ...
                self.NEW_INDEX.to_csv(index_file, index=True, encoding='gb2312')  # 保存索引记录
    ...
    ****可选参数：index_path：索引文件保存路径 [单位：]&[选项：]&[默认：None]。
    ...
    def get_index(self, info: dict) -> dict:
        ...
        try:  # 尝试读取索引文件，若索引文件存在，则将['循环段', '上升段起点', '稳定段起点', '稳定段终点']保存至Index_value中
            if self.RAW_INDEX.empty:  # 索引文件行位置记录
                self.RAW_INDEX = pd.read_csv(index_file, index_col=0, encoding='gb2312', dtype={'Normal': str})  # 获取索引
...
class TBM_SPLIT(object):
    ...
    ****必选参数：input_path：原始数据存放路径 [单位：]&[选项：]&[默认：None]。
                ...
                par_name：关键参数名称 [单位：]&[选项：]&[默认：None]。
    ****可选参数：index_path：索引文件保存路径 [单位：]&[选项：]&[默认：None]。
                partition：内部段划分方法 [单位：]&[选项：统计平均值'Average'/核密度估计'Kernel']&[默认：统计平均值'Average']。
                min_time：时间记录下限值 [单位：s]&[选项：]&[默认：200]。
                debug：程序调试/修复选项 [单位：]&[选项：True/False]&[默认：False]。
                Run：直接运行程序 [单位：]&[选项：True/False]&[默认：True]。
    """
    ...
    def __init__(self, input_path=None, out_path=None, index_path=None, parameter=None,
                 partition='Average', min_time=200, debug=False, Run=True):
        """初始化必要参量"""
        self.input = input_path  # 初始化输入路径
        self.out = out_path  # 初始化输出路径
    ...
    def write_index(self, info: dict) -> None:  # 规定_Num_为整型(int)，info为列表(list)，返回值返回值无类型限定
        ...
        ID = after_index.pop('循环段')  # 将'循环段'一列移动到最前面
        after_index.insert(0, '循环段', ID)  # 将'循环段'一列移动到最前面
        after_index.to_csv(index_file, index=False, encoding='gb2312')  # 保存索引记录
    ...
    def main(self) -> None:  # 规定返回值无类型限定/无返回值
        ...
        for num, file in enumerate(all_file):  # 遍历每个文件
            ...
            self.check_index(list(data))  # 检查参数是否重复
            RS_Index = self.custom_get_RS({'name': file, 'data': data})  # 调用自定义函数，获取空推、上升、稳定、下降的变化点
            if RS_Index is None:  # 优先调用自定义模块，若自定义模块未定义，则调用默认模块
                if self.partition == 'Average':  # 使用统计平均值模块进行划分
                    RS_Index = self.get_RS_index_average({'name': file, 'data': data})  # 调用函数，获取空推、上升、稳定、下降的变化点
                if self.partition == 'Kernel':  # 使用核密度估计模块进行划分
                    RS_Index = self.get_RS_index_kernel({'name': file, 'data': data})  # 调用函数，获取空推、上升、稳定、下降的变化点
            if RS_Index['稳定段起点'] - RS_Index['上升段起点'] >= 30 and RS_Index['稳定段终点'] - RS_Index['稳定段起点'] >= 50:
    ...
    def get_RS_index_average(self, data: dict) -> dict:
        """
    ...
    def get_RS_index_kernel(self, data: dict) -> dict:
        """
        ...
        return {'上升段起点': rise, '稳定段起点': steadyS, '稳定段终点': data['data'].shape[0] - steadyE}
...
class TBM_FILTER(object):
    ...
    ****可选参数：index_path：索引文件保存路径 [单位：]&[选项：]&[默认：None]。
    ...
    def check_parm(self) -> None:  # 规定返回值无类型限定/无返回值
        ...
        import sys
        Class = self.__class__.__name__  # 获取当前模块名称
        if (not self.input) or (not self.out) or (not self.parm) or ('--请选择--' in self.parm) or ('' in self.parm):
            print('-> %s \033[0;31mThe input/output/index path or parameter are faulty, Please check!!!\033[0m' % Class)
            sys.exit()  # 抛出异常并终止程序
    ...
    def custom_filter(self, data: DataFrame) -> ndarray:  # 规定data为DataFrame类型
        """
        自定义滤波器模块，若定义了相关功能，则优先运行此功能，若未定义相关功能，则运行默认滤波器模块
        :param data: 待滤波数据（DataFrame）
        :return: 滤波后数据（DataFrame）
        """
        pass
...
class TBM_REPORT(object):
    ...
    ****必选参数：input_path：原始数据存放路径 [单位：]&[选项：]&[默认：None]。
                ...
                par_name：关键参数名称 [单位：]&[选项：]&[默认：None]。
    ****可选参数：index_path：索引文件保存路径 [单位：]&[选项：]&[默认：None]。
                cover：添加封面 [单位：]&[选项：True/False]&[默认：True]。
                content：添加目录 [单位：]&[选项：True/False]&[默认：True]。
                header：添加页眉 [单位：]&[选项：True/False]&[默认：True]。
                footer：添加页脚 [单位：]&[选项：True/False]&[默认：True]。
                watermark：添加水印 [单位：]&[选项：True/False]&[默认：False]。
                watermark_info：水印名称 [单位：]&[默认：None]。
                pic_outer：使用外部图片资源 [单位：]&[选项：True/False]&[默认：False]。
                input_pic：外部图片存放路径 [单位：]&[默认：False]。
                debug：程序调试/修复选项 [单位：]&[选项：True/False]&[默认：False]。
                Run：直接运行程序 [单位：]&[选项：True/False]&[默认：True]。
    """
    import sys
    ...
    def __init__(self, input_path=None, out_path=None, index_path=None, parameter=None, input_pic=None,
                 cover=True, content=True, header=True, footer=True, watermark=False, pic_outer=False,
    ...
    def custom_plot(self, data: DataFrame, _key_: dict) -> pyplot:  # 规定_key_为字典类型（dict）,返回值为绘制好的plt
        ...
        :param data: 单个循环段数据（DataFrame）
        :param _key_: {'上升段起点': 上升段起点索引（int）, '稳定段起点': 稳定段起点索引（int）, '稳定段终点': 稳定段终点索引（int）}
        :return: 绘制的 plt 项目
        """
        pass
...
class TBM_PLOT(object):
    ...
    ****必选参数：input_path：原始数据存放路径 [单位：]&[选项：]&[默认：None]。
                out_path：生成数据保存路径 [单位：]&[选项：]&[默认：None]。
                par_name：关键参数名称 [单位：]&[选项：]&[默认：None]。
    ****可选参数：index_path：索引文件保存路径 [单位：mm]&[选项：]&[默认：None]。
                height：生成图片的高度 [单位：mm]&[选项：]&[默认：100mm]。
                weight：生成图片的宽度 [单位：mm]&[选项：]&[默认：80mm]。
                dpi：生成图片的分辨率 [单位：]&[选项：]&[默认：120dpi]。
                Format：生成图片的格式 [单位：]&[选项：svg/png]&[默认：png]。
                show：展示生成的图片 [单位：]&[选项：True/False]&[默认：False]。
                debug：程序调试/修复选项 [单位：]&[选项：True/False]&[默认：False]。
                Run：直接运行程序 [单位：]&[选项：True/False]&[默认：True]。
    """
    ...
    def __init__(self, input_path=None, out_path=None, index_path=None, parameter=None, height=100,
                 weight=80, dpi=120, Format='png', show=False, debug=False, Run=True):
        """初始化必要参量"""
        self.input = input_path  # 初始化输入路径
        self.out = out_path  # 初始化输出路径
        self.parm = parameter  # 掘进状态判定参数
        """初始化可选参量"""
        self.index = index_path  # 初始化索引文件路径
        self.size = (int(height / 10), int(weight / 10))  # 图片大小（100mm，80mm）
...
class MESSAGE(object):
    ...
    def __init__(self):
        ...
        try:
            self.example = {'Cycle': TBM_CYCLE.PARAMETERS, 'Extract': TBM_EXTRACT.PARAMETERS,
                            'Class': TBM_CLASS.PARAMETERS, 'Clean': TBM_CLEAN.PARAMETERS,
                            'Merge': TBM_MERGE.PARAMETERS, 'Filter': TBM_FILTER.PARAMETERS,
                            'Split': TBM_SPLIT.PARAMETERS, 'Plot': TBM_PLOT.PARAMETERS,
                            'Report': TBM_REPORT.PARAMETERS}  # 参数示例
        except (NameError, AttributeError):
            self.example = {'Cycle': None, 'Extract': None, 'Class': None, 'Clean': None, 'Merge': None,
                            'Filter': None, 'Split': None, 'Plot': None, 'Report': None}  # 参数示例
    ...
    def Text_Event(self, var):
        ...
        if self.example[var] is not None:
            Label(window, text=[parm for parm in (['参数示例：'] + self.example[var])], font=('宋体', 10)).pack(pady=7)
        else:
            Label(window, text='Error, Parameter example not found!', font=('宋体', 10)).pack(pady=7)
    ...
    def Backup_Recovery(self, Framework):
    ...
    def BRU_file(self, Type='backup'):
        ...
        if Type == 'backup':
            ...
            if self.update['backup'].get() != '--未选择任何文件--':
        ...
        if Type == 'recovery':
            if self.update['recovery'].get() != '--未选择任何文件--':
    ...
    def Get_Config(self, path='Resource\\config\\config.ini'):
        ...
        try:
            ...
            Index = dict([(str.upper(key), value) for key, value in parser.items('Index-file')])
            ...
            Index, Cycle, Extract, Class, Clean, Merge, Filter, Split, Plot, Report, update = self.Default_Setting()
        self.config = [Index, Cycle, Extract, Class, Clean, Merge, Filter, Split, Plot, Report, update]
        self.Setting_Var()
    ...
    def Main(self):
        ...
        self.Select_Button(group6_2, '  内部段划分方法  ', 'PARTITION-METHOD',
                           15, value=[["平均值  ", "核密度估计"], ['Average', 'Kernel']])
...
if __name__ == "__main__":  # 内部调用
    import sys
    print('-> \033[0;32mOperating System: %s  Current Program: %s  Run model: internal call\033[0m'
          % (sys.platform, os.path.basename(__file__)))
    ...
    if CONF[1]['USING-MODEL']:
        TBM_CYCLE(input_path=CONF[1]['INPUT-PATH'], out_path=CONF[1]['OUTPUT-PATH'], parameter=CONF[1]['PARAMETER'],
                  division=CONF[1]['DIVISION-WAY'], interval_time=CONF[1]['INTERVAL-TIME'], debug=CONF[1]['DEBUG'],
                  index_path=os.path.split(CONF[0]['INDEX-PATH'])[0], V_min=CONF[1]['VELOCITY-MIN'],
                  L_min=CONF[1]['LENGTH-MIN1'])
    if CONF[2]['USING-MODEL']:
        TBM_EXTRACT(input_path=CONF[2]['INPUT-PATH'], out_path=CONF[2]['OUTPUT-PATH'], key_parm=CONF[2]['PARAMETER'],
                    index_path=os.path.split(CONF[0]['INDEX-PATH'])[0], debug=CONF[2]['DEBUG'])
    if CONF[3]['USING-MODEL']:
        TBM_CLASS(input_path=CONF[3]['INPUT-PATH'], project_type=CONF[3]['PROJECT'], L_min=CONF[3]['LENGTH-MIN2'],
                  debug=CONF[3]['DEBUG'], out_path=CONF[3]['OUTPUT-PATH'], missing_ratio=CONF[3]['MISSING_RATIO'],
                  V_max=CONF[3]['VELOCITY-MAX1'], V_set_var=CONF[3]['VELOCITY-SET'], parameter=CONF[3]['PARAMETER'],
                  index_path=os.path.split(CONF[0]['INDEX-PATH'])[0])
    ...
    if CONF[5]['USING-MODEL']:
        TBM_MERGE(input_path=CONF[5]['INPUT-PATH'], out_path=CONF[5]['OUTPUT-PATH'], parameter=CONF[5]['PARAMETER'],
                  debug=CONF[5]['DEBUG'], index_path=os.path.split(CONF[0]['INDEX-PATH'])[0])
    if CONF[6]['USING-MODEL']:
        TBM_FILTER(input_path=CONF[6]['INPUT-PATH'], out_path=CONF[6]['OUTPUT-PATH'], parameter=CONF[6]['PARAMETER'],
                   debug=CONF[6]['DEBUG'], index_path=os.path.split(CONF[0]['INDEX-PATH'])[0])
    ...
    if CONF[8]['USING-MODEL']:
        TBM_PLOT(input_path=CONF[8]['INPUT-PATH'], out_path=CONF[8]['OUTPUT-PATH'], parameter=CONF[8]['PARAMETER'],
                 height=CONF[8]['HEIGHT'], weight=CONF[8]['WEIGHT'], dpi=CONF[8]['DPI'], Format=CONF[8]['FORMAT'],
                 show=CONF[8]['SHOW'], debug=CONF[8]['DEBUG'], index_path=os.path.split(CONF[0]['INDEX-PATH'])[0])
    if CONF[9]['USING-MODEL']:
        TBM_REPORT(input_path=CONF[9]['INPUT-PATH'], out_path=CONF[9]['OUTPUT-PATH'], parameter=CONF[9]['PARAMETER'],
                   ...
                   index_path=os.path.split(CONF[0]['INDEX-PATH'])[0])
    Finish = time.time()  # 获取当前时刻时间（用于计算程序执行时间）
    time = Finish - Start
    if time < 3600:
        if time < 60:
...
if __name__ != "__main__":  # 外部调用
    import sys
    print('-> \033[0;32mOperating System: %s  Current Program: %s  Run model: external call\033[0m'
          % (sys.platform, os.path.basename(__file__)))
    Check_Update()  # 检查程序是否存在新版本
