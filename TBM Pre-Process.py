...
# * Last  update: 2023-01-16 00:00:00                                    *
# * License:  LGPL v1.0                                                  *
...
class TBM_CYCLE(object):
    ...
    ****可选参数：相邻循环段时间间隔（s）（interval_time），若不传入参数，则采用默认100s
    ...
    INDEX_NAME = ['循环段', '桩号', '日期', '掘进时间']
    NEW_INDEX = pd.DataFrame(columns=INDEX_NAME)  # 索引数据保存

    def __init__(self, input_path=None, out_path=None, parameter=None, division='Rotate Speed',
                 interval_time=100, V_min=1, L_min=10, debug=False, Run=True):
        """初始化各参量"""
        ...
        self.debug = DEBUG(Title=self.__class__.__name__, Debug=debug)  # 调试/修复程序
    ...
    def check_parm(self) -> None:  # 规定返回值无类型限定/无返回值
        ...
        if (not self.input) or (not self.out) or (not self.parm) or (len(self.parm) < len(self.PARAMETERS)):  # 检查必要参数
            print(' -> %s' % Class, '\033[0;31mThe input/output path or parameter are faulty, Please check!!!\033[0m')
            sys.exit()  # 抛出异常并终止程序

    def check_index(self, all_location):
        """判断数据中是否存在重复列"""
        if reduce(lambda x, y: x * y, [type(name) == str for name in self.parm]):  # 判断是否为标签类型索引
    ...
    def write_index(self, _info_: dict) -> None:  # 规定_Num_为整型(int)，_info_为列表(list)，返回值返回值无类型限定
        """索引文件写入数据  _info_中至少含有{‘name’：‘’}"""
        if self.debug.debug:
            return None
        global index_path  # 索引文件路径
        if self.NEW_INDEX.empty:
            if os.path.isfile(index_path):
                os.remove(index_path)  # 若索引文件存在，则删除
        Num = int(_info_['name'][:5])  # 当前文件编号
        self.NEW_INDEX.loc[Num] = [Num] + [_info_[name] for name in self.INDEX_NAME[1:]]  # 新的索引文件记录
        self.NEW_INDEX.to_csv(index_path, index=False, encoding='gb2312')  # 保存索引记录
    ...
    def tunnel_determine(self, _data_np_: Series) -> [bool, str]:  # 规定_data_np_为Numpy类型数组(Series)，返回值类型为[布尔值，字符型]
        ...
        Fx = np.int64(_data_np_[0:1] > 0.1)  # 判定函数 F(x) = 1(x>0), 0(x<=0)
        if self.division == 'Advance Speed':
            Fx = np.int64(_data_np_[1:2] > 0.1)  # 判定函数 F(x) = 1(x>0), 0(x<=0)
    ...
    def cycle_splice(self, _data_: DataFrame) -> DataFrame:  # 规定_data_为DataFrame类型数组(DataFrame)，返回数组类型为DataFrame
        ...
        _data_np_ = _data_.iloc[:, self.parm[3:5]].values  # 提取与掘进状态判定有关的参数，并对其类型进行转换，以提高程序执行速度
        mark = 0
        ...
        _out_data_ = pd.concat([self.add_temp, _data_.iloc[:mark + 1, :]], ignore_index=True)  # 将两天连续部分数据进行拼接，形成一个数据集
        self.add_temp = _data_.iloc[mark + 1:, :]  # 将当天与后一天连续部分的数据进行单独保存，便于和后一天的数据进行拼接
    ...
    def cycle_extract(self, _data_: DataFrame) -> None:  # 规定_data_为DataFrame类型数组(DataFrame)，返回值无类型限定
        ...
        data_np = _data_.iloc[:, self.parm[2:5]].values  # 提取与掘进状态判定有关的参数，并对其类型进行转换，以提高程序执行速度
        for index, item in enumerate(data_np):  # 从前往后，对每一时刻的掘进状态进行判定（index为行索引， item为每行数据）
            ...
            if (key['now-F'] > key['now-S']) or (index == len(data_np) - 1):  # 获取到掘进段开始和结束索引是否完整
                Length = abs(data_np[key['now-F'], 0] - data_np[key['now-S'], 0])  # 掘进长度大于10mm
                if (index == len(data_np) - 1) or Length > self.L_min and max(data_np[key['now-S']:key['now-F'], 2]) > self.V_min:
    ...
    @staticmethod  # 不强制要求传递参数
    def custom_read(file_path: str) -> DataFrame:  # 规定file_path为字符型数据，返回值为（DataFrame）类型数组
    ...
    def main(self) -> None:  # 规定返回值无类型限定/无返回值
        ...
        visual = VISUAL(File=len(file_list), Title='Boring-cycle extract', Debug=self.debug)  # 可视化
        for num, file in enumerate(file_list):  # 遍历每个文件
            self.file = file
            local_path = os.path.join(self.input, file)  # 当前文件路径
            data_raw = self.custom_read(local_path) if self.custom_read(local_path) else self.default_read(local_path)
            ...
            self.debug.print(Name=file, Start=False, Draw=True, Data=after_process, Parm=[self.parm[3]], Key=None)
            visual.Print_info()  # 可视化

    def cycle_save(self, _data_: DataFrame, Start: int, Finish: int) -> None:  # 规定_data_为(DataFrame)类型数组，返回值无类型限定
        """对已经划分出的循环段数据进行保存"""
        self.debug.print(Start=False, Finish=False, Key=['Start:', Start, 'Finish:', Finish])  # DEBUG输出
...
class TBM_EXTRACT(object):
    ...
    ****可选参数：程序调试/修复选项（debug），默认为关闭状态
    ...
    def main(self) -> None:  # 规定返回值无类型限定/无返回值
        ...
        visual = VISUAL(File=len(file_name_list), Title='Key-Data extract', Debug=self.debug)  # 可视化
        for num, file in enumerate(file_name_list):  # 遍历每个文件
            ...
            self.debug.print(Title='Key Extract', Start=False, Draw=False, Count=1, Name='---',
                             Key=['   %s:    %d\n' % (list(data)[name], name) for name in self.parm])  # DEBUG输出
    ...
    ****可选参数：程序调试/修复选项（debug），默认为关闭状态
    ...
    def write_index(self, _info_: dict) -> None:  # 规定_Num_为整型(int)，_info_为列表(list)，返回值返回值无类型限定
        ...
        if self.RAW_INDEX.empty:  # 索引文件行位置记录
            if os.path.isfile(index_path):
                self.RAW_INDEX = pd.read_csv(index_path, index_col=0, encoding='gb2312')  # 读取索引文件
        ...
        local_Num = int(_info_['name'][:5])  # 当前文件编号
        ...
        self.NEW_INDEX.loc[local_Num] = [local_Num] + [_info_[name] for name in self.INDEX_NAME[1:]]  # 新的索引文件记录
        ...
        after_index.to_csv(index_path, index=False, encoding='gb2312')  # 保存索引记录
    ...
    def A_premature(self, _cycle_: DataFrame) -> str:  # 规定_cycle_为DataFrame类型数组(DataFrame)，返回值类型为字符型(str)
        ...
        start_length = _cycle_.iloc[0, self.parm[2]]  # 获取循环段开始点位移,推进位移（self.parm[2]）
        end_length = _cycle_.iloc[_cycle_.shape[0] - 1, self.parm[2]]  # 获取循环段结束点位移,推进位移（self.parm[2]）
    ...
    def B1_markAndModify(self, _cycle_: DataFrame) -> str:  # 规定_cycle_为DataFrame类型数组(DataFrame)，返回值类型为字符型(str)
        ...
        data_V = _cycle_.iloc[:, self.parm[4]].values  # 获取推进速度并转化类型，推进速度（self.parm[4]）
    ...
    def B2_constant(self, _cycle_: DataFrame) -> str:  # 规定_cycle_为DataFrame类型数组(DataFrame)，返回值类型为字符型(str)
        ...
        data_F = _cycle_.iloc[:, self.parm[6]].values  # 获取刀盘推力并转化类型，刀盘推力（self.parm[6]）
    ...
    def C1_sine(self, _cycle_: DataFrame) -> str:  # 规定_cycle_为DataFrame类型数组(DataFrame)，返回值类型为字符型(str)
        ...
        data_T = _cycle_.iloc[:, self.parm[5]]  # 获取刀盘扭矩，刀盘扭矩（self.parm[5]）
        T_mean = data_T.mean()
    ...
    def C2_shutdown(self, _cycle_: DataFrame) -> str:  # 规定_cycle_为DataFrame类型数组(DataFrame)，返回值类型为字符型(str)
        ...
        N_set = _cycle_.iloc[:, self.parm[8]].values  # 获取刀盘转速设定值，刀盘转速设定值（self.parm[8]）
        V_set = _cycle_.iloc[:, self.parm[9]].values  # 获取推进速度设定值，推进速度设定值（self.parm[9]）
    ...
    def D_adjust_setting(self, _cycle_: DataFrame) -> str:  # 规定_cycle_为DataFrame类型数组(DataFrame)，返回值类型为字符型(str)
        ...
        data_V = _cycle_.iloc[:, self.parm[4]]  # 获取推进速度，推进速度（self.parm[4]）
        ...
        data_V_set = (_cycle_.iloc[:, self.parm[9]] * self.PROJECT_COEFFICIENT[self.project_type]).std()  # 获取推进速度设定值的方差
    ...
    def E_missing_ratio(self, _cycle_: DataFrame) -> str:  # 规定_cycle_为DataFrame类型数组(DataFrame)，返回值类型为字符型(str)
        ...
        data_time = _cycle_.iloc[:, self.parm[1]].values  # 获取日期并转化类型，日期（self.parm[1]）
    ...
    def main(self) -> None:  # 规定返回值无类型限定/无返回值
        ...
        visual = VISUAL(File=len(csv_list), Title='Data-Class', Debug=self.debug)  # 可视化
        for cycle, file in enumerate(csv_list):
            ...
            RS_Index = self.custom_get_RS(data_A) if self.custom_get_RS(data_A) else self.default_get_RS(data_A)  # 获取RS
            result = copy.deepcopy(self.RESULT)
            result['A'] = 'True' if self.A_premature(data_A) == 'A' else ''  # 调用相关模块判断是否为A类异常
            if RS_Index['上升段起点'] < RS_Index['稳定段起点'] < RS_Index['稳定段终点']:
                ...
                result['B1'] = 'True' if self.B1_markAndModify(data_RS) == 'B1' else ''  # 调用相关模块判断是否为B1类异常
            ...
            self.debug.print(Parm=self.parm[3:7], Start=False, Finish=False, Sum=len(csv_list), Data=data_A,
                             Name=file, Key=['循环段分类:'] + ['' if result[key] not in ['True', 'Unknown'] else key
                             if result[key] != 'Unknown' else 'Unknown' for key in result.keys()], Draw=True)  # DEBUG输出
            visual.Print_info()  # 可视化

    def default_get_RS(self, _data_) -> dict:
        """使用默认方式获取空推、上升、稳定、下降的关键点"""
        try:
            if _data_.shape[0] < 80:
                RS_index = {'上升段起点': 0, '稳定段起点': 0, '稳定段终点': 0}
                return RS_index
            T_mean = _data_.iloc[:, self.parm[5]].mean()
            mid_start_value = min(_data_.iloc[0:int(_data_.shape[0] / 3), self.parm[9]])  # 找前1/3的v_set最小值
            mid_point_start = 0  # 中点位置索引
            while _data_.iloc[mid_point_start, self.parm[9]] > mid_start_value:
                mid_point_start += 1
            V_set_mean = _data_.iloc[mid_point_start:-1, self.parm[9]].mean()  # 第一次求均值，用于索引中点
            mid_point = mid_point_start
            while _data_.iloc[mid_point, self.parm[9]] < V_set_mean:
                if mid_point > 0.7 * _data_.shape[0]:
                    mid_point = int(_data_.shape[0] * 0.2)
                else:
                    while _data_.iloc[mid_point, self.parm[9]] < V_set_mean or \
                            _data_.iloc[mid_point + 30, self.parm[9]] < V_set_mean:
                        mid_point += 1  # #############有修改
            steadyE = _data_.shape[0] - 1  # 稳定段结束位置索引
            while _data_.iloc[steadyE, self.parm[5]] <= T_mean:  # 判断稳定段结束点
                steadyE -= 1
            if mid_point and mid_point <= 0.7 * _data_.shape[0]:
                rise = mid_point  # 上升段开始位置处索引
                while abs(_data_.iloc[rise, self.parm[7]]) > 0 and rise > 10:  # 刀盘贯入度度索引（self.parm[7]） ########
                    rise -= 1
                steadyS = mid_point
                V_set_ = _data_.iloc[mid_point_start:steadyE, self.parm[9]]  # #改改改改改改改改改改改改改改改
    ...
    def custom_get_RS(self, _data_: DataFrame) -> dict:
...
class TBM_CLEAN(object):
    ...
    ****可选参数：程序调试/修复选项（debug），默认为关闭状态
    ...
    def write_index(self, _info_: dict) -> None:  # 规定_Num_为整型(int)，_info_为列表(list)，返回值返回值无类型限定
        ...
        try:
            if self.RAW_INDEX.empty:  # 索引文件行位置记录
                ...
                self.RAW_INDEX = pd.read_csv(index_path, index_col=0, encoding='gb2312', dtype=Type)  # 读取索引文件
                self.NEW_INDEX = copy.deepcopy(self.RAW_INDEX)  # 复制一份索引文件
            self.RAW_INDEX[_info_['Type']][int(_info_['name'][:5])] = ''  # 修正后去掉异常循环段标识
            if (self.RAW_INDEX.loc[int(_info_['name'][:5]), self.INDEX_NAME[1:-1]] == 'True').sum() <= 0:
                self.NEW_INDEX['Normal'][int(_info_['name'][:5])] = 'True'  # 将修正后的索引修改为True
                self.NEW_INDEX.to_csv(index_path, index=True, encoding='gb2312')  # 保存索引记录
    ...
    def B1_mark_Modify(self, _cycle_: DataFrame) -> DataFrame:  # 规定_cycle_为(DataFrame)类型数组，返回值为(DataFrame)类型数组
        ...
        data_V = _cycle_.iloc[:, self.parm[4]].values  # 推进速度（self.parm[4]）
        data_mean, data_std = np.mean(data_V), np.std(data_V)  # 获取推进速度均值和标准差
        for i in range(len(data_V)):
            if data_V[i] > self.V_threshold_value or (data_V[i] > data_mean + 3 * data_std):
                ...
                _cycle_.iloc[i, self.parm[4]] = replace  # 采用前10个推进速度的平均值进行替换
        return _cycle_
    ...
    def main(self) -> None:  # 规定返回值无类型限定/无返回值
        ...
        visual = VISUAL(Dir=len(os.listdir(self.input)), Title='Data-Clean', Debug=self.debug)  # 可视化
        for Key, Dir in zip(list(self.SUB_FOLDERS.keys()), os.listdir(self.input)):
            csv_list = os.listdir(os.path.join(self.input, Dir))  # 获取输入文件夹下的所有文件夹名称，并将其保存
            csv_list.sort(key=lambda x: int(x[:5]))  # 对读取的文件列表进行重新排序
            visual.Print_info(File=len(csv_list))
            for cycle, file in enumerate(csv_list):  # 遍历输入文件夹下的所有子文件夹
                ...
                function = {'A': self.A_premature_Modify(data), 'B1': self.B1_mark_Modify(data),
                            ...
                            'E': self.E_missing_ratio_Modify(data), 'Unknown': None, 'Normal': None}
                after_data = function[Key]  # 调用相对应的修正函数对异常数据进行修正
                if after_data is not None:
                    after_data.to_csv(target_csv_path, index=False, encoding='gb2312')  # 保存修正后'A'类循环段数据
                    self.write_index({'name': file, 'Type': Key})  # 修正索引文件
                    self.debug.print(Parm=self.parm[3:7], Start=False, Finish=False, Data=data,
                                     Key=['循环段修正：'] + ['%s  ->  修正为  ->  Normal' % Key], Name=file, Draw=True)  # DEBUG
                    visual.Print_info()  # 可视化
                if Key == 'Normal':
...
class TBM_MERGE(object):
    ...
    ****可选参数：程序调试/修复选项（debug），默认为关闭状态
    ...
    def check_parm(self) -> None:  # 规定返回值无类型限定/无返回值
        """检查传入参数是否正常"""
        Class = self.__class__.__name__  # 获取当前模块名称
        if (not self.input) or (not self.out):  # 检查必要参数
            print(' -> %s' % Class, '\033[0;31mThe input/output path are faulty, Please check!!!\033[0m')
            sys.exit()  # 抛出异常并终止程序

    def get_index(self, _info_: dict) -> dict:
        """获取索引文件记录  _info_中至少含有{‘name’：‘’}"""
        global index_path  # 索引文件路径
        try:  # 尝试读取索引文件，若索引文件存在，则将['循环段', '上升段起点', '稳定段起点', '稳定段终点']保存至Index_value中
            if self.RAW_INDEX.empty:  # 索引文件行位置记录
                self.RAW_INDEX = pd.read_csv(index_path, index_col=0, encoding='gb2312', dtype={'Normal': str})  # 获取索引
                self.RAW_INDEX = self.RAW_INDEX.loc[:, 'Normal']  # 将关键点保存至Index_value
            mark = '' if self.RAW_INDEX is None else self.RAW_INDEX[int(_info_['name'][:5])]
    ...
    def main(self) -> None:  # 规定返回值无类型限定/无返回值
        ...
        visual = VISUAL(Dir=len(os.listdir(self.input)), Title='Data-Merge', Debug=self.debug)  # 可视化
        for Dir in os.listdir(self.input):  # 遍历输入路径下的所有子文件夹
            Dir_path = os.path.join(self.input, Dir)  # 子文件夹路径
            visual.Print_info(File=len(os.listdir(Dir_path)))  # 可视化
...
class TBM_SPLIT(object):
    ...
    ****可选参数：程序调试/修复选项（debug），默认为关闭状态
    ...
    SUB_FOLDERS = {'空推段': 'Free running', '上升段': 'Loading', '稳定段': 'Boring',
                   '上升和稳定段': 'Loading and Boring', '循环段': 'Boring cycle'}
    PARAMETERS = ['桩号', '日期', '推进位移', '刀盘转速', '推进速度', '刀盘扭矩',
                  '总推力', '刀盘贯入度', '刀盘转速设定值', '推进速度设定值']
    INDEX_NAME = ['循环段', '上升段起点', '稳定段起点', '稳定段终点']
    RAW_INDEX = pd.DataFrame()
    NEW_INDEX = pd.DataFrame(columns=INDEX_NAME)  # 索引数据保存

    def __init__(self, input_path=None, out_path=None, parameter=None, partition='kernel density', min_time=200,
    ...
    def segment_save(self, _data_, _key_: dict) -> None:  # 规定_name_为字符型（str），_key_为字典类型（dict）
        """对已经分割好的循环段文件进行保存  示例：_key_：{‘name’：‘’，‘rise’：‘’，‘steadyS’：‘’，‘steadyE’：‘’}"""
        save_data = {'空推段': _data_.iloc[:_key_['上升段起点'], :],  # 空推段数据
                     '上升段': _data_.iloc[_key_['上升段起点']:_key_['稳定段起点'], :],  # 上升段数据
                     '稳定段': _data_.iloc[_key_['稳定段起点']:_key_['稳定段终点'], :],  # 稳定段数据
                     '上升和稳定段': _data_.iloc[_key_['上升段起点']:_key_['稳定段终点'], :],  # 上升段和稳定段数据
                     '循环段': _data_}  # 整个循环段数据
        for Key in list(self.SUB_FOLDERS.keys()):
            save_path = os.path.join(self.out, self.SUB_FOLDERS[Key], _key_['name'])
            save_data[Key].to_csv(save_path, index=False, encoding='gb2312')  # 保存数据
    ...
    def main(self) -> None:  # 规定返回值无类型限定/无返回值
        ...
        file_name_list = os.listdir(self.input)  # 获取输入文件夹下的所有文件名，并将其保存
        file_name_list.sort(key=lambda x: int(x[:5]))  # 对读取的文件列表进行重新排序
        visual = VISUAL(File=len(file_name_list), Title='Data_Split', Debug=self.debug)  # 可视化
        for num, file in enumerate(file_name_list):  # 遍历每个文件
            ...
            self.check_index(list(data))  # 检查参数是否重复
            RS_Index = self.custom_get_RS(data)  # 调用自定义函数，获取空推、上升、稳定、下降的变化点
            if RS_Index is None:  # 优先调用自定义模块，若自定义模块未定义，则调用默认模块
                if self.partition == 'Average':
                    RS_Index = self.get_RS_index_average(data)  # 调用函数，获取空推、上升、稳定、下降的变化点
                if self.partition == 'Kernel Density':
            ...
            self.debug.print(Key=['上升段起点:', RS_Index['上升段起点'], '稳定段起点:', RS_Index['稳定段起点'],
                                  '稳定段终点:', RS_Index['稳定段终点']], Start=False, Finish=False, Sum=len(file_name_list),
                             Draw=True, Data=data, Parm=[self.parm[4]], Name=file)  # DEBUG输出
            visual.Print_info()  # 可视化

    def get_RS_index_average(self, _data_: DataFrame) -> dict:
        """获取空推、上升、稳定、下降的关键点--(类别1)"""
        try:
            if _data_.shape[0] < self.min_time:
                RS_index = {'上升段起点': 0, '稳定段起点': 0, '稳定段终点': 0}
                return RS_index
            T_mean = _data_.iloc[:, self.parm[5]].mean()
            mid_start_value = min(_data_.iloc[0:int(_data_.shape[0] / 3), self.parm[9]])  # 找前1/3的v_set最小值
    ...
    def get_RS_index_kernel(self, _data_: DataFrame) -> dict:
        ...
        def get_index(data, ratio, limit, short=False):
            ...
            for row in range(3, len(raw_V) - 3):
                ...
                if finish - start > ratio * len(raw_V):
                    while First or (Steady - Rise >= limit * len(raw_V) >= 5):
                        ...
                        KDE_Diff_mean = abs(
                            float(kde.support[np.where(kde.density == np.max(kde.density))]))  # 推进速度差值的均值
        ...
        if _data_.shape[0] >= self.min_time >= 200:
            rise, steadyS = get_index(_data_.iloc[:, self.parm[4]], ratio=0.1, limit=0.4)
            _, steadyE = get_index(_data_.iloc[::-1, self.parm[4]], ratio=0.05, limit=0.02, short=True)
            return {'上升段起点': rise, '稳定段起点': steadyS, '稳定段终点': _data_.shape[0] - steadyE}
        else:
            print(' -> ', '\033[0;31mThe minimum time record should be greater than 200s!!!\033[0m')
            sys.exit()  # 抛出异常并终止程序
...
class TBM_FILTER(object):
    ...
    ****可选参数：关键参数名称（par_name），若传入参数，则仅对传入的参数进行降噪，若不传入参数，则对所有参数进行降噪
    ...
    @staticmethod  # 不强制要求传递参数
    def default_filter(_data_: DataFrame) -> ndarray:  # 规定_data_为DataFrame类型，返回值为 array 类型
        """巴特沃斯滤波器"""
        return scipy.signal.savgol_filter(_data_, 19, 4)  # 巴特沃斯滤波器，滑动窗口长度为19，滤波阶数为4

    @staticmethod  # 不强制要求传递参数
    def custom_filter(_data_: DataFrame) -> ndarray:  # 规定_data_为DataFrame类型
    ...
    def main(self) -> None:  # 规定返回值无类型限定/无返回值
        ...
        visual = VISUAL(File=len(file_name_list), Title='Data filter', Debug=self.debug)  # 可视化
        for num, file in enumerate(file_name_list):  # 遍历每个文件
            ...
            for col in self.parm:
                if not isinstance(data.iloc[0, col], str):
                    after_filter = self.custom_filter(data.iloc[:, col]) if self.custom_filter(data.iloc[:, col]) \
                        else self.default_filter(data.iloc[:, col])  # 参数滤波
                    data.iloc[:, col] = after_filter
            data.to_csv(os.path.join(self.out, file), index=False, encoding='gb2312')  # 保存滤波后的循环段数据
            self.debug.print(Start=False, Finish=False, Sum=len(file_name_list), Key=['滤波参数: '] + ([list(data)[i]
                             for i in self.parm] if len(self.parm) != len(list(data)) else ['全部参数']), Name=file)
            visual.Print_info()  # 可视化
...
class TBM_REPORT(object):
    ...
    ****可选参数：程序调试/修复选项（debug），默认为关闭状态
    ...
    """
    import sys
    if os.path.exists('Resource/fonts/STSONG.TTF'):
        pdfmetrics.registerFont(TTFont('SimSun', 'Resource/fonts/STSONG.TTF'))
    else:
        print(' ->', '\033[0;31mFont file(STSONG.TTF) not found, You can download the font and place it in '
                     '...\\Resource\\fonts!!!\033[0m')
        sys.exit()  # 抛出异常并终止程序
    ...
    PARAMETERS = ['桩号', '日期', '推进位移', '刀盘转速', '推进速度', '刀盘扭矩',
                  '总推力', '刀盘贯入度', '刀盘转速设定值', '推进速度设定值']
    INDEX_NAME = ['循环段', '上升段起点', '稳定段起点', '稳定段终点']
    RAW_INDEX = pd.DataFrame()  # 索引文件记录

    def __init__(self, input_path=None, out_path=None, parameter=None, input_pic=None, cover=True, content=True,
                 header=True, footer=True, watermark=True, pic_outer=False, watermark_info=None, debug=False, Run=True):
        """初始化各参量"""
    ...
    def add_footer_info(self, Obj) -> None:  # 规定返回值无类型限定/无返回值:
        """添加页脚信息"""
        Obj.setFont(self.type_font, self.size_font)  # 设置字体类型及大小
        Obj.setFillColor(colors.black)  # 设置字体颜色为黑色
        Obj.drawString(102 * mm, 12 * mm, f'Page%d' % self.page)  # 添加页脚信息
    ...
    def add_header_info(self, Obj) -> None:
        ...
        Obj.drawString(35 * mm, 282 * mm, 'REPORT')  # 添加页脚信息
        current_date = datetime.datetime.now().strftime('%Y.%m')
        Obj.setFont(self.type_font, self.size_font + 1)  # 设置字体类型及大小
        Obj.setFillColor(colors.royalblue)  # 设置字体颜色为黑色
        Obj.drawString(150 * mm, 282 * mm, f'%s  <数据预处理>  |  %d' % (current_date, self.page))  # 添加页脚信息
    ...
    def add_watermark_info(self, Obj) -> None:
        ...
        Obj.setFillAlpha(0.2)  # 设置透明度，1为不透明
        Obj.drawString(55 * mm, 4 * mm, self.watermark_context)  # 添加水印名称
        Obj.drawString(185 * mm, 4 * mm, self.watermark_context)  # 添加水印名称
        Obj.drawString(123 * mm, 85 * mm, self.watermark_context)  # 添加水印名称
        Obj.drawString(250 * mm, 85 * mm, self.watermark_context)  # 添加水印名称
    ...
    def add_cover_info(self, Obj) -> None:
        ...
        sheet.wrapOn(Obj, 0, 0)  # 将sheet添加到Canvas中
        sheet.drawOn(Obj, 30 * mm, 50 * mm)  # 将sheet添加到Canvas中
        if os.path.exists(self.cover_path):  # 若路径有效，则添加图片
            Obj.drawImage(image=self.cover_path, x=20 * mm, y=120 * mm, width=170 * mm, height=120 * mm, anchor='c')

    def add_content_info(self, Obj, _value_: dict, _place_: int) -> None:  # 规定返回值无类型限定/无返回值
        """添加目录信息"""
        if _place_ == 0:
            sheet = Table([['CATALOGUE']], colWidths=[136 * mm], rowHeights=[8 * mm],
                          style={("FONT", (0, 0), (-1, -1), self.type_font, self.size_font + 5),  # 目录标题字体
                                 ("TEXTCOLOR", (0, 0), (-1, -1), colors.black),  # 字体颜色
                                 ('ALIGN', (0, 0), (-1, -1), 'CENTER'), ('VALIGN', (0, 0), (-1, -1), 'MIDDLE')})  # 居中
            sheet.wrapOn(Obj, 0, 0)  # 将sheet添加到Canvas中
            sheet.drawOn(Obj, 36 * mm, 280 * mm)  # 将sheet添加到Canvas中
        table_y = [(274.9 - 5.1 * i) * mm for i in range(50)]  # 表格列宽信息
        sheet = Table([['%s-%s' % (_value_['beg'], _value_['end']), '', _value_['stake'],
                        '.' * 150, 'Page%s' % _value_['page']]],
                      colWidths=[16 * mm, 3 * mm, 10 * mm, 95 * mm, 12 * mm], rowHeights=[5.1 * mm],
                      style={("FONT", (0, 0), (-1, -1), self.type_font, self.size_font),  # 目录正文字体
                             ("TEXTCOLOR", (0, 0), (-1, -1), colors.black),  # 字体颜色
                             ('ALIGN', (0, 0), (-1, -1), 'CENTER'), ('VALIGN', (0, 0), (-1, -1), 'MIDDLE')})  # 左右上下居中
        sheet.wrapOn(Obj, 0, 0)  # 将sheet添加到Canvas中
        sheet.drawOn(Obj, 36 * mm, table_y[_place_])  # 将sheet添加到Canvas中

    def add_text_info(self, Obj, _value_: list, _place_: int) -> None:  # 规定返回值无类型限定/无返回值
        """添加正文信息"""
        table_x = [20 * mm, 105 * mm, 20 * mm, 105 * mm, 20 * mm, 105 * mm]  # 表格位置信息
        table_y = [189 * mm, 189 * mm, 105 * mm, 105 * mm, 21 * mm, 21 * mm]  # 表格位置信息
        _value_.append(['' for _ in range(6)])
        sheet = Table(data=_value_,
        ...
        sheet.wrapOn(Obj, 0, 0)  # 将sheet添加到Canvas中
        sheet.drawOn(Obj, table_x[_place_], table_y[_place_])  # 将sheet添加到Canvas中

    def add_pic_info(self, Obj, _value_: 'list | DataFrame', _place_: int) -> None:  # 规定返回值无类型限定/无返回值
        """添加图片信息"""
        pic_x = [21 * mm, 106 * mm, 21 * mm, 106 * mm, 21 * mm, 106 * mm]
        pic_y = [190 * mm, 190 * mm, 106 * mm, 106 * mm, 22 * mm, 22 * mm]
        if isinstance(_value_, pd.DataFrame):
            img = self.draw_pic(_value_)  # 使用内置绘图模块进行绘图
            Obj.drawImage(image=img, x=pic_x[_place_], y=pic_y[_place_], width=83 * mm, height=66 * mm, anchor='c')
        elif isinstance(_value_, str):
            img = _value_  # 添加外部图片资源
            if os.path.exists(img):  # 若路径有效，则添加图片
                Obj.drawImage(image=img, x=pic_x[_place_], y=pic_y[_place_], width=83 * mm, height=66 * mm, anchor='c')

    def draw_Canvas(self, pdf_text, pdf_content, _data_: DataFrame, _cycle_: int, _name_: str, finish: bool) -> None:
        """调用各模块"""
        data_np = _data_.iloc[:, self.parm[:3]].values  # 提取与掘进状态判定有关的参数，并对其类型进行转换，以提高程序执行速度
        text_value = [['Number', ('%00005d' % _cycle_),  # 获取循环段编号(Num)
        ...
        self.add_text_info(pdf_text, text_value, _cycle_ % 6 - 1)  # 添加正文信息
        pic_val = os.path.join(self.input_pic, _name_[:-3] + 'png')  # 图片路径信息
        if not self.pic_outer:  # 是否采用外部绘图模块添加图片资源
            pic_val = _data_
        if finish:
            _cycle_ += 6
        self.add_pic_info(pdf_text, pic_val, _cycle_ % 6 - 1)  # 添加图片信息
        if _cycle_ % 6 == 1:
            self.content_value.update({'beg': text_value[0][1], 'stake': text_value[0][3][:-1], 'page': self.page})
        if _cycle_ % 6 == 0 or finish:
            self.content_value.update({'end': text_value[0][1]})
            if self.switch_content:
                self.add_content_info(pdf_content, self.content_value, (int(_cycle_ / 6)) % 50 - 1)  # 添加目录信息
        ...
        if _cycle_ % 300 == 0 or finish:
            pdf_content.showPage()  # 目录新增一页

    def draw_pic(self, data: DataFrame):
        """内部绘图模块"""
        Mark = self.get_index({'name': self.file})
        Plt = self.custom_plot(data, Mark) if self.custom_plot(data, Mark) else self.default_plot(data, Mark)  # 绘图
    ...
    def get_index(self, _info_: dict) -> dict:
        ...
        try:  # 尝试读取索引文件，若索引文件存在，则将['循环段', '上升段起点', '稳定段起点', '稳定段终点']保存至Index_value中
            ...
            mark = {} if self.RAW_INDEX is None else {'上升段起点': self.RAW_INDEX['上升段起点'][int(_info_['name'][:5])],
                                                      '稳定段起点': self.RAW_INDEX['稳定段起点'][int(_info_['name'][:5])],
                                                      '稳定段终点': self.RAW_INDEX['稳定段终点'][int(_info_['name'][:5])]}
        except (FileNotFoundError, KeyError, IndexError, TypeError):  # 若索引文件存在，则令Index_value为空[]
            mark = {}
        return mark

    def default_plot(self, _data_: DataFrame, _key_: dict) -> pyplot:  # 规定_key_为字典类型（dict）,返回值为绘制好的plt
        """完成掘进参数的绘图与保存"""
        x = [i for i in range(_data_.shape[0])]  # 'Time'
        y_n = _data_.iloc[:, self.parm[3]]  # '刀盘转速'
        y_n_set = _data_.iloc[:, self.parm[8]]  # '刀盘转速设定'
        y_V = _data_.iloc[:, self.parm[4]]  # '推进速度'
        y_V_set = _data_.iloc[:, self.parm[9]]  # '推进速度设定'
        y_T = _data_.iloc[:, self.parm[5]]  # '刀盘总扭矩'
        y_F = _data_.iloc[:, self.parm[6]]  # '推进总推力'
        ...
        if _key_:  # 如果索引文件中空推段、上升段和稳定段的变化点存在，则在图中绘出
            plt.axvline(x=_key_['上升段起点'], c="r", ls="-.")
            plt.axvline(x=_key_['稳定段起点'], c="r", ls="-.")
            plt.axvline(x=_key_['稳定段终点'], c="r", ls="-.")
        return plt

    def custom_plot(self, _data_: DataFrame, _key_: dict) -> pyplot:  # 规定_key_为字典类型（dict）,返回值为绘制好的plt
    ...
    def main(self) -> None:  # 规定返回值无类型限定/无返回值
        ...
        cover_path = os.path.join(self.out, 'cover.pdf')  # 封面pdf存储路径
        ...
        pdf_cover = Canvas(filename=cover_path, bottomup=1, pageCompression=1, encrypt=None)  # 创建目录pdf
        file_list = os.listdir(self.input)  # 获取循环段列表
        file_list.sort(key=lambda x: int(x[:5]))  # 对读取的文件列表进行重新排序
        visual = VISUAL(File=int(len(file_list) / 6), Title='Create PDF', Debug=self.debug)  # 可视化
        use_file = []  # DEBUG时用到
        for cycle, file in enumerate(file_list):
            self.file, finish = file, False  # 文件名
            use_file.append('\n   ' + file)  # DEBUG时用到
            try:  # 尝试采用编码'gb2312'读取数据
            ...
            self.check_index(list(data))  # 检查参数是否重复
            if cycle == len(file_list) - 1:
                finish = True
            self.draw_Canvas(pdf_text, pdf_content, data, cycle + 1, file, finish)
            if (cycle + 1) % 6 == 0 or cycle == len(file_list) - 1:
                None if (cycle + 1) % 6 != 0 else visual.Print_info()  # 可视化
                self.debug.print(Start=False, Finish=False, Sum=int((len(file_list) + 6) / 6), Draw=False,
                                 Key=['图片来源: ', '外部图片' if self.pic_outer else '内部图片', '\n   文件列表:'] + use_file)  # DEBUG
                use_file = []  # DEBUG时用到
        None if not self.switch_cover else self.add_cover_info(pdf_cover)  # 添加目录信息
        pdf_text.save(), pdf_content.save(), pdf_cover.save()  # pdf保存
        self.MergePDF([cover_path, content_path, text_path])  # 合并目录和正文
    ...
    def MergePDF(self, pdf_path: list) -> None:  # 规定返回值无类型限定/无返回值
        ...
        for file in pdf_path:
            ...
            os.remove(file)
        merger.write(os.path.join(self.out, 'TBM-Data.pdf'))
        merger.close()
...
class TBM_PLOT(object):
    ...
    ****可选参数：程序调试/修复选项（debug），默认为关闭状态
    ...
    def __init__(self, input_path=None, out_path=None, parameter=None, height=10, weight=8, dpi=120, Format='png',
                 show=False, debug=False, Run=True):
        """初始化各参量"""
        self.input = input_path  # 初始化输入路径
        self.out = out_path  # 初始化输出路径
        self.parm = parameter  # 掘进状态判定参数
    ...
    def default_plot(self, _data_: DataFrame, _key_: dict) -> pyplot:  # 规定_key_为字典类型（dict）,返回值为绘制好的plt
        ...
        plt.ylabel("刀盘推力、刀盘扭矩", fontsize=20, labelpad=10)
        if _key_:  # 如果索引文件中空推段、上升段和稳定段的变化点存在，则在图中绘出
            plt.axvline(x=int(_key_['上升段起点']), c="r", ls="-.")
            plt.axvline(x=int(_key_['稳定段起点']), c="r", ls="-.")
            plt.axvline(x=int(_key_['稳定段终点']), c="r", ls="-.")
        return plt
    ...
    def main(self) -> None:  # 规定返回值无类型限定/无返回值
        ...
        file_list.sort(key=lambda x: int(x[:5]))  # 对读取的文件列表进行重新排序
        visual = VISUAL(File=len(file_list), Title='Drawing pic', Debug=self.debug)  # 可视化
        for file in file_list:  # 遍历每个文件
            local_csv_path = os.path.join(self.input, file)  # 当前循环段数据存放路径
            try:  # 尝试采用编码'gb2312'读取数据
                data = pd.read_csv(local_csv_path, encoding='gb2312')  # 读取循环段数据，编码'gb2312'
            except UnicodeDecodeError:  # 若采用编码'gb2312'读取数据失败，则尝试采用默认编码读取数据
...
class VISUAL(object):
    ...
    def __init__(self, Debug, Title, File=1, Dir=1):
        self.Time_val = []
        self.Print = Title
        self.Sum_file = File
        self.Sum_dir = Dir
    ...
    def Print_info(self, File=None):
        if File is not None:
            self.Sum_file = File
            self.Count_dir = self.Count_dir + 1 if self.Sum_file == 0 else self.Count_dir
            return
        if self.Debug.debug:
            return
        ...
        print('\r', '->', '[第%d个 / 共%d个]  ' % (self.Count_file, self.Sum_file), '[所用时间%ds / 平均时间%ds]'
              % (int(time_diff), int(mean_time)), ' ', '[CPU占用: %5.2f%% / 内存占用: %5.2f%%]'
              % (cpu_percent, mem_percent), '  ', '\033[0;33m累积时间:%6.3f小时\033[0m' % sum_time, end='')
        if self.Count_file >= self.Sum_file and self.Count_dir >= self.Sum_dir:
            sum_time = round(sum(self.Time_val) / 3600, 3)  # 计算程序执行的总时间
            print('\r', '->', '\033[0;32m%s completed, which took %6.3f hours\033[0m' % (self.Print, sum_time))
        self.Count_dir = self.Count_dir + 1 if self.Count_file == self.Sum_file else self.Count_dir
        self.Count_file = 0 if self.Count_file >= self.Sum_file else self.Count_file
        self.Count_file += 1
        self.time = time.time()
...
class MESSAGE(object):
    ...
    def __init__(self):
        ...
        self.Dir = {'Cycle': ['', '[ TBM_CYCLE ]-AllDataSet'],
                    ...
                    'Filter': ['[ TBM_MERGE ]-ML-Data', '[ TBM_SPLIT ]-A-ML2-Data'],
                    'Split': ['[ TBM_MERGE ]-ML-Data', '[ TBM_FILTER ]-ML2-Data'],
                    'Plot': ['[ TBM_MERGE ]-ML-Data', '[ TBM_PLOT ]-Pic'],
                    'Report': ['[ TBM_MERGE ]-ML-Data', '[ TBM_REPORT ]-Pdf-Data']}  # 输入输出文件夹路径
        self.example = {'Cycle': TBM_CYCLE(Run=False).PARAMETERS, 'Extract': TBM_EXTRACT(Run=False).PARAMETERS,
                        'Class': TBM_CLASS(Run=False).PARAMETERS, 'Clean': TBM_CLEAN(Run=False).PARAMETERS,
                        'Merge': TBM_MERGE(Run=False).PARAMETERS, 'Filter': TBM_FILTER(Run=False).PARAMETERS,
                        'Split': TBM_SPLIT(Run=False).PARAMETERS, 'Plot': TBM_PLOT(Run=False).PARAMETERS,
                        'Report': TBM_REPORT(Run=False).PARAMETERS}  # 参数示例
    ...
    def Input_Event(self, var, fuc=None):
        """获取输入路径"""
        newDir = tkinter.filedialog.askdirectory(initialdir='D:\\', title='打开目录').replace('/', '\\')
        if not newDir:
            return
        Directory = newDir.replace(newDir.split('\\')[-1], '')
        if fuc is None:
            self.Var_input[var].set(newDir)
            self.Var_output[var].set(os.path.join(Directory, self.Dir[var][1]))
            self.Var_index.set(os.path.join(Directory, 'Index-File.csv'))
        else:
            fuc[var].set(newDir)
        if var == 'Cycle':
            for model, Var in zip(list(self.Dir.keys())[1:], list(self.Dir.values())[1:]):
                self.Var_input[model].set(os.path.join(Directory, Var[0]))
                self.Var_output[model].set(os.path.join(Directory, Var[1]))
            self.Var_other['PLOT-PATH'].set(os.path.join(Directory, self.Dir['Plot'][1]))

    def Output_Event(self, var):
        """获取输出路径"""
        newDir = tkinter.filedialog.askdirectory(initialdir='D:\\', title='另存目录').replace('/', '\\')
        if not newDir:
            return
    ...
    def Bottom_Button(self, Framework):
        tkinter.Button(Framework, text='确 定', command=lambda: self.Ok_Event(),
                       font=self.Text).place(x=309, y=715, width=70, height=25)
        tkinter.Button(Framework, text='取 消', command=lambda: self.Cancel_Event(),
                       font=self.Text).place(x=409, y=715, width=70, height=25)
        tkinter.Button(Framework, text='应 用', command=lambda: self.Apply_Event(),
                       font=self.Text).place(x=509, y=715, width=70, height=25)
    ...
    def Default_Setting():
        ...
        return Index_path, Cycle, Extract, Class, Clean, Merge, Filter, Split, Plot, Report
    ...
    def Creative_Config(self):
        ...
        with open('Resource\\config\\config.ini', 'w+', encoding='GBK') as cfg:
            Config.write(cfg)
    ...
    def Get_Config(self):
        ...
        try:
            parser.read('Resource\\config\\config.ini', encoding='GBK')  # 读取文件
            ...
            Index_path, Cycle, Extract, Class, Clean, Merge, Filter, Split, Plot, Report = self.Default_Setting()
        self.config = [Index_path, Cycle, Extract, Class, Clean, Merge, Filter, Split, Plot, Report]
        self.Setting_Var()
    ...
    def Main(self):
        ...
        style.configure('TNotebook.Tab', font=('宋体', 13), foreground='black')
        ...
        tkinter.Button(self.root, text='版本信息', command=lambda: self.Version_Event(),
                       font=self.Text).place(x=7, y=645, width=70, height=25)
        tkinter.Button(self.root, text='恢复默认', command=lambda: self.Reset_Event(),
                       font=self.Text).place(x=7, y=680, width=70, height=25)
        tkinter.Button(self.root, text=' 帮 助 ', command=lambda: self.Help_Event(),
                       font=self.Text).place(x=7, y=715, width=70, height=25)

        fr1 = Frame(self.root)  # 2 创建选项卡1的容器框架
        notebook.add(fr1, text=' CYCLE ')  # 3 装入框架1到选项卡1
        ...
        notebook.add(fr2, text='EXTRACT')  # 3 装入框架1到选项卡1
        ...
        notebook.add(fr3, text=' CLASS ')  # 3 装入框架1到选项卡1
        ...
        notebook.add(fr4, text=' CLEAN ')  # 3 装入框架1到选项卡1
        ...
        notebook.add(fr5, text=' MERGE ')  # 3 装入框架1到选项卡1
        ...
        notebook.add(fr6, text=' SPLIT ')  # 3 装入框架1到选项卡1
        ...
        notebook.add(fr7, text=' FILTER')  # 3 装入框架1到选项卡1
        ...
        notebook.add(fr8, text='  PLOT ')  # 3 装入框架1到选项卡1
        ...
        notebook.add(fr9, text=' REPORT')  # 3 装入框架1到选项卡1
...
class DEBUG(object):
    ...
    def print(self, Start=True, Finish=True, Sum=float('inf'), Count=float('inf'), Draw=False, Name=None, **Var):
        """语句打印输出"""
        if self.debug:
            if self.Number <= Count:
                if Start or self.Number == 1:
                    print('=' * 40, 'Detail Information', '=' * 40)
                    print('\033[0;33m', '·' * int((96 - len(self.title)) / 2), '%s' % self.title,
                          '·' * int((96 - len(self.title)) / 2), '\033[0m')
                if Var['Key'] is not None:
                    if not self.divide:
                        self.divide = [[] for _ in range(len(Var['Key']) + 2)]
                    for num, information in enumerate(
                            [' Cycle:', '  %s\n' % (Name if Name is not None else self.Number)] + Var['Key']):
                        if isinstance(information, int) and information is not None:
                            self.divide[int(num / 2) - 1].append(information)
                        print('\033[0;33m%9s\033[0m' % information, end='')
                    print('')
                if Finish or self.Number >= Sum:
                    print('\033[0;33m', '·' * int((96 - len(self.title)) / 2), '%s' % self.title,
                          '·' * int((96 - len(self.title)) / 2), '\033[0m')
                    print('=' * 40, 'Detail Information', '=' * 40)
                if Draw:
                    self.draw(Var['Data'], Var['Parm'])
                    self.divide = []
            self.Number += 1

    def draw(self, Data, Parms):
        """绘图"""
        x = [i for i in range(Data.shape[0])]  # 'Time'
        plt.figure(figsize=(14, 7), dpi=120)  # 设置画布大小（10cm x 8cm）及分辨率（dpi=120）
        for parm, color in zip(Parms, ['b', 'g', 'r', 'y']):
            plt.plot(x, Data.iloc[:, parm], label="%s" % list(Data)[parm], color=color)
        plt.legend()
        plt.xlabel("时间/s", fontsize=15)
        for color, information in zip(['g', 'r', 'y', 'b'], self.divide):
...
if __name__ == "__main__":
    ...
    if CONF[4]['USING-MODEL']:
        TBM_CLEAN(input_path=CONF[4]['INPUT-PATH'], out_path=CONF[4]['OUTPUT-PATH'], debug=CONF[4]['DEBUG'],
                  parameter=CONF[4]['PARAMETER'], V_max=CONF[4]['VELOCITY-MAX2'])
