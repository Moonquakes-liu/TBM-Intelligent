...
backup_switch = False
resource = {'config.ini': 'config\\config.ini',
            'STSONG.TTF': 'fonts\\STSONG.TTF',
            'Support.docx': 'helps\\Support.docx',
            'Support.pdf': 'helps\\Support.pdf',
...
def Unpack_file(target_path, file):
    ...
    if not os.path.exists(zip_file):
        return
    if os.path.exists(os.path.join(root_path, 'Resource', resource[file])):
        backup_file(file_path=os.path.join(root_path, 'Resource', resource[file]),
                    now_vision='%s' % os.path.splitext(file)[-1])
    ZF = zipfile.ZipFile(zip_file, mode='r')
    for num, name in enumerate(ZF.namelist()):
        if os.path.basename(name) == file:
        ...
        try:
            if Operate_Type == 'update-file':
                ...
                if os.path.exists(zip_file):
                    ...
                    Unpack_file(target_path=resource_path, file=Operate_File)
                    update_info += ['%-20s: %s' % (Operate_File, info) for
                                    info in parser[info]['INFORMATION'].split('；')]
