...
def delete_info(**Var):
    ...
    for line in datafile:
        if Var['target_message'] in line:
            print(' ->->',
                  '\033[0;33mDelete information from %s Successfully! Location: %s lines\033[0m' % (name, local + 1))
            break
        local += 1
...
def update_file(**Var):
    ...
    print(' ->->', '\033[0;33mUpdate %s Successfully! Version: %s ->-> %s\033[0m' %
...
def backup_file(**Var):
    ...
    backup_switch = True
    name = os.path.splitext(Var['file_path'].split('\\')[-1])[0] + ' %s.bak' % Var['now_vision']
    file = os.path.abspath(os.path.join(Var['file_path'], os.path.pardir))
    now_file = Var['file_path']  # 当前文件夹路径
    backups_file = os.path.join(file, 'Resource', 'backups', name)
