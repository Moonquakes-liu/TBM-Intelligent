...
class TBM_SPLIT(object):
    ...
    def segment_save(self, _data_, _file_, _Index_):
        ...
        if _Index_['rise'] > 0 and Time_Loading > 30 and Time_Boring > 50:  # 空推段持续时间不为0，上升段持续时间>30s，稳定段持续时间>50s
            ...
            Free_running_df.to_csv(os.path.join(self.out_path + self.DIR_OUT[0], _file_), index=False, encoding='gb2312')
            Loading_df.to_csv(os.path.join(self.out_path + self.DIR_OUT[1], _file_), index=False, encoding='gb2312')
            Boring_df.to_csv(os.path.join(self.out_path + self.DIR_OUT[2], _file_), index=False, encoding='gb2312')
            Loading_Boring_df.to_csv(os.path.join(self.out_path + self.DIR_OUT[3], _file_), index=False, encoding='gb2312')
            _data_.to_csv(os.path.join(self.out_path + self.DIR_OUT[4], _file_), index=False, encoding='gb2312')

    def data_Split(self, _input_path_, _out_path_, _par_name_, _dir_path_):
        """对数据进行整体和细部分割"""
        if (not _input_path_) and (not _out_path_) and (not _par_name_) and (not _dir_path_):  # 检查传入参数是否正常
            ...
            sys.exit()
        self.out_path, self.parameter, self.DIR_OUT = _out_path_, _par_name_, _dir_path_
        ...
        visual(-1, len(file_name_list), None, None, 'Data_Split', True)
...
def butter_worth_filter(_input_path_, _out_path_, N=2, Wc=0.2):
    ...
    visual(-1, len(file_name_list), None, None, 'Data filter', True)
