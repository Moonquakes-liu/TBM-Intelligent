...
Out_Folder = ['(TBM_CYCLE)-AllDataSet', '(TBM_CYCLE)-KeyDataSet', '(TBM_CLASS)-Class-Data', '(TBM_CLASS)-Clean-Data',
              '(TBM_CLASS)-ML-Data', '(TBM_SPLIT)-ML2-Data', '(TBM_SPLIT)-A-ML2-Data', '(TBM_REPORT)-Pdf-Data',
              '(TBM_REPORT)-Pic']  # 生成目录
...
TBM_CYCLE().read_file(Raw_Data_Input, Cycle_Data_Output, 100, Par_name)  # 调用相关模块
...
key_parameter_extraction(Cycle_Data_Output, Key_Data_Output, key_name)  # 调用相关模块

# ->->模块3：数据分类<-<-
par_name = []  # 待添加
Out_dir = ['\\1-A1class-data', '\\2-B1class-data', '\\3-B2class-data', '\\4-C1class-data',
           '\\5-C2class-data', '\\6-D1class-data', '\\7-E1class-data', '\\8-Norclass-data', ]  # 子文件夹
TBM_CLASS().data_class(Key_Data_Output, Class_Data_Output, par_name, Out_dir)  # 调用相关模块

# ->->模块4、5：异常1数据清理、异常2数据清理<-<-
par_name = []  # 待添加
Out_dir = ['\\1-NorA1class-data', '\\2-NorB1class-data', '\\3-NorB2class-data', '\\4-NorC1class-data',
           '\\5-NorC2class-data', '\\6-NorD1class-data', '\\7-NorE1class-data', '\\8-Norclass-data', ]  # 子文件夹
TBM_CLEAN().data_clean(Class_Data_Output, Clean_Data_Output, par_name, Out_dir)  # 调用相关模块

# ->->模块6：合并形成机器学习数据集<-<-
ML_data(Clean_Data_Output, ML_Data_Output)  # 调用相关模块

# ->->模块7：数据降噪<-<-
butter_worth_filter(Key_Data_Output, ML2_Data_Output)  # 调用相关模块

# ->->模块8：内部段分割<-<-
Par_name = ['推进位移', '推进速度(nn/M)', '刀盘贯入度', '推进给定速度']  # (推进位移、推进速度、刀盘贯入度、推进速度设定值)请勿修改顺序
Out_dir = ['\\1-Free running', '\\2-Loading', '\\3-Boring', '\\4-Loading and Boring', '\\5-Boring cycle']  # 子文件夹
TBM_SPLIT().data_Split(Key_Data_Output, A_ML2_Data_Output, Par_name, Out_dir)  # 调用相关模块

# ->->模块10：数据绘图<-<-
Par_name = ['刀盘转速', '刀盘给定转速', '推进速度(nn/M)', '推进给定速度', '刀盘扭矩', '总推力']  # （要绘制的掘进参数）请勿修改顺序
plot_parameters_TBM(Key_Data_Output, Pic_Data_Output, Par_name)  # 调用相关模块
