...
class TBM_CYCLE(object):
    def __init__(self):
        ...
        self.state_parameter = parameters  # 掘进状态判定参数
    ...
    def read_file(self, _input_path_, _out_path_, _interval_time_, _par_name_):
        """完成对原始数据的读取"""
        if (not _input_path_) and (not _out_path_) and (not _interval_time_) and (not _par_name_):  # 检查传入参数是否正常
        ...
        visual(-1, len(file_name_list), None, None, 'Boring-cycle extract', True)
...
def key_parameter_extraction(_input_path_, _out_path_, key_name):
    """关键数据提取"""
    if (not _input_path_) and (not _out_path_) and (not key_name):  # 检查传入参数是否正常
        print('->-> %s' % os.path.basename(__file__), '\033[0;31mParameters abnormal, Please check!!!\033[0m')
        sys.exit()
    ...
    visual(-1, len(file_name_list), None, None, 'Key-Data extract', True)
